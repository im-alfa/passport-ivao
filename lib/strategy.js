...
const IvaoConstants = require('./constants').IvaoConstants;

/**
 * Creates an instance of `IvaoStrategy`.
 *
 * IVAO Strategy is a modification of OpenID Connect strategy that authenticates requests
 * using OpenID Connect, which is an identity layer on top of the OAuth 2.0 protocol.
 *
...
function Strategy(options, verify) {
  ...
  if (!verify) { throw new TypeError('IvaoStrategy requires a verify function'); }
  if (!options.clientID) { throw new TypeError('IvaoStrategy requires a clientID option'); }
  if (!options.clientSecret) { throw new TypeError('IvaoStrategy requires a clientID option'); }
  ...
  this._oauth2 = new OAuth2(options.clientID,  options.clientSecret,
                            '', IvaoConstants.authorizationURL, IvaoConstants.tokenURL, options.customHeaders);
  this._oauth2.useAuthorizationHeaderforGET(true);
  if (options.agent) {
    this._oauth2.setAgent(options.agent);
  }
  
  this._issuer = IvaoConstants.issuer;
  ...
  console.log(IvaoConstants.authorizationURL);
  var key = options.sessionKey || (this.name + ':' + url.parse(IvaoConstants.authorizationURL).hostname);
  this._stateStore = options.store || new SessionStateStore({ key: key });
  
  this._userInfoURL = IvaoConstants.userInfoURL;
...
Strategy.prototype.authenticate = function(req, options) {
  ...
  var meta = {
    issuer: IvaoConstants.issuer,
    authorizationURL: IvaoConstants.authorizationURL,
    tokenURL: IvaoConstants.tokenURL,
